...
export const ReviewsPage: React.FC = () => {
  ...
  const fetchReviews = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${API_BASE}/getfeedbacks`, {credentials: 'include'});
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ message, rate, user_id: currentUserId })
      });
  ...
  const filteredReviews = activeFilter === 'all'
    ? reviews
    : reviews.filter(r => r.rate === activeFilter);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200/60 pb-8 mb-10 gap-6">
          <div className="flex items-start gap-4">
            ...
            >
              <svg xmlns="http://w3.org" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
            </button>
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Отзывы пользователей</h1>
              <p className="text-sm font-medium text-slate-500 mt-1">Прямая синхронизация с базой данных</p>
            </div>
          </div>
            ...
            {filteredReviews.map((review) => (
              <div key={review.id} className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-xl shadow-indigo-100/10 flex flex-col justify-between hover:shadow-indigo-100/30 transition-all">
                <div>
                  ...
                  <div className="flex items-center gap-0.5 mb-3 text-amber-400">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <svg key={i} className={`w-4 h-4 ${i < review.rate ? 'fill-current' : 'text-slate-200'}`} viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                    ))}
                  </div>
                  ...
                  {currentUserId === review.user_id && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100/80 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                      >
                        Ред.
                      </button>
                      <button
                        onClick={() => handleDelete(review.id)}
                        className="text-xs font-semibold text-rose-600 hover:text-rose-800 bg-rose-50 hover:bg-rose-100/80 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                      >
                        Удалить
                      </button>
                    </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Что вам понравилось или что нужно улучшить в CRM?"
                ...
                <button
                  type="submit"
                  className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold py-3 px-4 rounded-xl transition-all cursor-pointer text-center text-sm shadow-md shadow-indigo-100 hover:shadow-lg hover:shadow-indigo-200"
                >
